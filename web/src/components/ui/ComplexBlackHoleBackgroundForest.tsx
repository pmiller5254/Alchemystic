...
const COLORS = [
    ...
    '#6d4c1b', // earthy brown
    '#b45309', // golden brown
    '#fef9c3', // pale sunlight
];

export default function ComplexBlackHoleBackgroundForest({ scrollProgress = 0 }: ComplexBlackHoleBackgroundProps) {
    ...
    useEffect(() => {
        ...
        function drawWavyDisk(ctx: CanvasRenderingContext2D, centerX: number, centerY: number, minDim: number, elapsed: number) {
            ...
            ctx.closePath();
            // Fill with a forest gradient
            const grad = ctx.createRadialGradient(centerX, centerY, baseInner, centerX, centerY, baseOuter + waveAmplitude);
            grad.addColorStop(0, 'rgba(34,197,94,0.10)'); // vibrant green
            grad.addColorStop(0.3, 'rgba(163,230,53,0.18)'); // lime green
            grad.addColorStop(0.6, 'rgba(109,76,27,0.22)'); // earthy brown
            grad.addColorStop(0.85, 'rgba(22,101,52,0.22)'); // deep forest green
            grad.addColorStop(1, 'rgba(180,83,9,0.18)'); // golden brown
